...
int main(int argc, char** argv) {
  ...
  const int pathLength = std::snprintf(
      ...
      "/signal_controller_response_%ld", static_cast<long>(getpid()));
  if (pathLength < 0 ||
  ...
  ipc_dropin::Socket<sizeof(RunTargetResponse), kResponseSocketCapacity>
      responseSocket{};
  // Lifecycle's default Linux sandbox runs managed processes as uid 1000,
  // while this CLI may be invoked by root on the development machine. The
  // response endpoint therefore needs cross-uid read/write access. Requests
  // remain protected by the daemon-owned 0600 control endpoint.
  ipc_dropin::ReturnCode responseCreateResult{};
  {
    // shm_open applies the process umask to its mode. Temporarily clear it so
    // 0666 is not silently reduced to 0644 when this CLI is run by root.
  ...
  // Verify that a live daemon consumed the message instead of accepting a
  // connection to stale shared memory left by an abnormal previous run.
  ...
  // Wait for the asynchronous ControlClient result so this CLI's exit code
  // represents the actual Run Target transition.
    ...
    if (state == ipc_dropin::ReturnCode::kOk) {
      if (response.success) {
        if (std::strcmp(argv[1], kStopCommand) == 0) {
          ...
          std::cout << "[LMCONTROL][SUCCESS] signal controller system "
                       "stopped\n";
          return EXIT_SUCCESS;
        }
