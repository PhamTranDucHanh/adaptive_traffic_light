...
bool sendActivationResponse(const std::string& responseSocketPath,
  ...
  if (responseSocket.connect(responseSocketPath.c_str()) !=
    ...
    std::cerr << "[SIGNAL_CONTROL_DAEMON][RESPONSE][ERROR] could not send "
                 "result socket="
              << responseSocketPath << '\n';
    return false;
  }
...
int main() {
  ...
  // A State_Manager is managed by Launch Manager and must report ready before
  // it is permitted to request Run Target transitions.
  score::mw::lifecycle::report_running();
  std::cout << "[SIGNAL_CONTROL_DAEMON][LIFECYCLE] reported Running to "
               "Launch Manager\n";
  ...
  if (controlSocket.create(kControlSocketPath, 0600) !=
      ipc_dropin::ReturnCode::kOk) {
    std::cerr << "[SIGNAL_CONTROL_DAEMON][INIT][ERROR] could not create "
                 "socket="
              << kControlSocketPath << '\n';
    return EXIT_FAILURE;
  }
  ...
  std::cout << "[SIGNAL_CONTROL_DAEMON][READY] pid=" << getpid()
  ...
  while (!exitRequested.load()) {
    ...
    if (controlSocket.tryReceive(request) == ipc_dropin::ReturnCode::kOk) {
      ...
      if (runTargetName == kStopCommand) {
        // Launch Manager forks this daemon directly. The deployment wrapper
        // uses exec so the daemon's parent PID remains the Launch Manager PID.
        const pid_t launchManagerPid = getppid();
        if (!processExists(launchManagerPid)) {
          const std::string errorMessage{
              "Launch Manager parent process is not available"};
          std::cerr << "[SIGNAL_CONTROL_DAEMON][STOP][ERROR] "
        ...
        std::cout << "[SIGNAL_CONTROL_DAEMON][REQUEST] stop "
                     "launch_manager_pid="
                  << launchManagerPid << '\n';

        // Acknowledge first because Launch Manager will terminate this daemon
        // as part of the managed shutdown immediately after SIGTERM.
        ...
        std::cout << "[SIGNAL_CONTROL_DAEMON][STOP] requesting Launch "
                     "Manager shutdown signal=SIGTERM pid="
                  << launchManagerPid << '\n';
        if (kill(launchManagerPid, SIGTERM) != 0) {
          std::cerr << "[SIGNAL_CONTROL_DAEMON][STOP][ERROR] kill(SIGTERM) "
                       "failed: "
                    << std::strerror(errno) << '\n';
        }
        continue;
      ...
      std::cout << "[SIGNAL_CONTROL_DAEMON][REQUEST] activate run_target="
                << runTargetName << '\n';
      ...
      controlClient.ActivateRunTarget(runTargetName).Then(
          {callbackScope,
           [runTargetName, responseSocketPath](auto& result) noexcept {
             if (result) {
               std::cout << "[SIGNAL_CONTROL_DAEMON][SUCCESS] active "
                            "run_target="
                         << runTargetName << '\n';
               (void)sendActivationResponse(responseSocketPath, true, "");
             } else {
               const std::string errorMessage{result.error().Message()};
               std::cerr << "[SIGNAL_CONTROL_DAEMON][ERROR] activation "
                            "failed run_target="
  ...
  std::cout << "[SIGNAL_CONTROL_DAEMON][STOP] shutdown complete\n";
  return EXIT_SUCCESS;
}
