...
void YOLOv8Backend::postprocess(const cv::Mat& frame, const std::vector<Ort::Value>& output_tensors, InferenceResult& result) {
    ...
    // ---------------------------------------------------------------
    if (frameCount_ == 0) {
        std::cout << "[YOLOv8Backend][probe] Output tensor rank=" << shape.size() << "\n";
        for (std::size_t s = 0; s < shape.size(); ++s) {
            std::cout << "[YOLOv8Backend][probe]   shape[" << s << "]="
                      << shape[s] << "\n";
        }
        std::cout << "[YOLOv8Backend][probe] First 10 raw values:";
        for (int v = 0; v < 10; ++v) {
            std::cout << " " << data[v];
        }
        std::cout << "\n";
    ...
    // Layout is determined at runtime from shape[1] (see layout detection block below).
    // Transpose/Reshape logic: [1, Features, Anchors] -> [Anchors, Features] (row-major)
    cv::Mat output(static_cast<int>(shape[1]), static_cast<int>(shape[2]),
                   CV_32F, const_cast<float*>(data));
    ...
    const int  num_classes_actual = static_cast<int>(shape[1]) - 4
                                    - (has_objectness ? 1 : 0);
    ...
    for (int i = 0; i < transposed.rows; ++i) {
        ...
        if (frameCount_ < 5 && i < 20) {
            std::cout << "[YOLOv8Backend][anchor" << i << "]";
            if (has_objectness) {
                std::cout << " obj=" << obj_score;
            } else {
                std::cout << " obj=N/A(LayoutA)";
            }
            std::cout << " classId=" << class_id
                      << " bestScore=" << best_class_score
        ...
        if (confidence > confThreshold_) {
            ...
            if (frameCount_ < 5) {
                std::cout << "[YOLOv8Backend][debug] Decoded box before undo transform: ["
                          << x1 << ", " << y1 << ", " << x2 << ", " << y2 << "]\n";
            }
            ...
            if (frameCount_ < 5) {
                std::cout << "[YOLOv8Backend][debug] Decoded box after undo transform (before clip): ["
                          << x1 << ", " << y1 << ", " << x2 << ", " << y2 << "]\n";
            }
            ...
            if (frameCount_ < 5) {
                std::cout << "[YOLOv8Backend][debug] Decoded box after undo transform (after clip/round): ["
                          << left_val << ", " << top_val << ", " << right_val << ", " << bottom_val << "]\n"
                          << "[YOLOv8Backend][debug] ratio=" << ratio_ << ", dw=" << dw_ << ", dh=" << dh_ << "\n";
            }
    ...
    for (int idx : indices) {
        ...
        Detection d = populateDetection(boxes[idx], classIds[idx],
                                        confidences[idx], isVehicle, isEmergency);

        if (frameCount_ < 5) {
            std::cout << "[YOLOv8Backend][debug] ClassId=" << d.ClassId
    ...
    if (frameCount_ < 5) {
        std::cout << "[YOLOv8Backend] Motorcycles detected: " << motorcycleCount << "\n"
