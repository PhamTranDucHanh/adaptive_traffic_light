...
static std::string resolveOiv7ClassName(int classId) {
    ...
    switch (classId) {
        case  42: return "Bicycle";
        case  73: return "Bus";
        case  90: return "Car";
        case 342: return "Motorcycle";
        case 558: return "Truck";
        case 564: return "Van";
        default:  return "Unknown_" + std::to_string(classId);
    }
}
...
InferenceResult YoloV8OIV7Backend::infer(const Frame& frame) {
    ...
    try {
        ...
        auto output_tensors = session_->Run(
            ...
            (const char* const*)input_node_names_.data(), &input_tensor, 1,
            (const char* const*)output_node_names_.data(), 1);
...
void YoloV8OIV7Backend::preprocess(const cv::Mat& frame, std::vector<float>& input_tensor_values) {
    ...
    if (frame.cols != new_unpad_w || frame.rows != new_unpad_h) {
        cv::resize(frame, resized, cv::Size(new_unpad_w, new_unpad_h), 0, 0, cv::INTER_LINEAR);
    } else {
        resized = frame;
    }
...
void YoloV8OIV7Backend::postprocess(const cv::Mat& frame, const std::vector<Ort::Value>& output_tensors, InferenceResult& result) {
    ...
    // Ultralytics YOLOv8 ONNX exports (including OIV7 600-class models [1, 604, 8400])
    // output format: [1, 4 + num_classes, num_anchors] with NO objectness score.
    // Layout A (YOLOv8 standard): shape[1] = 4 (bbox) + num_classes.
    // Layout B (legacy/custom):   shape[1] = 4 (bbox) + 1 (objectness) + num_classes (e.g. 85).
    const bool has_objectness = (shape[1] == 85);
    const int numClasses = static_cast<int>(shape[1]) - 4 - (has_objectness ? 1 : 0);
    const int numAnchors = static_cast<int>(shape[2]);

    if (frameCount_ < 5) {
        std::cout << "[YoloV8OIV7Backend][debug] Input tensor shape: [1, 3, 640, 640]\n";
        std::cout << "[YoloV8OIV7Backend][debug] Output tensor shape: [";
        for (std::size_t s = 0; s < shape.size(); ++s) {
            std::cout << shape[s] << (s + 1 < shape.size() ? ", " : "");
        }
        std::cout << "]\n";
        std::cout << "[YoloV8OIV7Backend][debug] has_objectness: " << (has_objectness ? "true" : "false") << "\n";
        std::cout << "[YoloV8OIV7Backend][debug] numClasses: " << numClasses << "\n";
        std::cout << "[YoloV8OIV7Backend][debug] numAnchors: " << numAnchors << "\n";
    }

    cv::Mat output(static_cast<int>(shape[1]), static_cast<int>(shape[2]), CV_32F, const_cast<float*>(data));
    ...
    for (int i = 0; i < transposed.rows; ++i) {
        ...
        if (frameCount_ == 0 && i < 10) {
            std::cout << "[YoloV8OIV7Backend][anchor " << i << "]"
                      << " cx=" << cx << " cy=" << cy << " w=" << w << " h=" << h
                      << " max_class_id=" << class_id << " max_class_score=" << best_class_score
                      << " confidence=" << confidence << " first 5 scores: ";
            for (int k = 0; k < std::min(5, numClasses); ++k) {
                std::cout << class_scores[k] << " ";
            }
            std::cout << "\n";
        }
        ...
        if (confidence > confThreshold_) {
            ...
            if (frameCount_ < 5) {
                std::cout << "[YoloV8OIV7Backend][debug] Decoded box before undo transform: ["
                          << x1 << ", " << y1 << ", " << x2 << ", " << y2 << "]\n";
            }
            ...
            if (frameCount_ < 5) {
                std::cout << "[YoloV8OIV7Backend][debug] Decoded box after undo transform (before clip): ["
                          << x1 << ", " << y1 << ", " << x2 << ", " << y2 << "]\n";
            }
            ...
            if (frameCount_ < 5) {
                std::cout << "[YoloV8OIV7Backend][debug] Decoded box after undo transform (after clip/round): ["
                          << left_val << ", " << top_val << ", " << right_val << ", " << bottom_val << "]\n"
                          << "[YoloV8OIV7Backend][debug] ratio=" << ratio_ << ", dw=" << dw_ << ", dh=" << dh_ << "\n";
            }
    ...
    if (frameCount_ < 5) {
        std::cout << "[YoloV8OIV7Backend][stats] global_max_score: " << global_max_score
                  << " (max_class_id: " << max_class_id_found << ")\n";
        std::cout << "[YoloV8OIV7Backend][stats] global_avg_score: "
                  << (total_score_count > 0 ? (sum_scores / total_score_count) : 0.0) << "\n";
        std::cout << "[YoloV8OIV7Backend][stats] anchors > 0.10: " << count_gt_010 << "\n";
        std::cout << "[YoloV8OIV7Backend][stats] anchors > 0.25: " << count_gt_025 << "\n";
        std::cout << "[YoloV8OIV7Backend][stats] anchors > 0.50: " << count_gt_050 << "\n";
        std::cout << "[YoloV8OIV7Backend][stats] boxes before NMS: " << boxes.size() << "\n";
    }
    ...
    if (frameCount_ < 5) {
        std::cout << "[YoloV8OIV7Backend][debug] Detections before NMS: " << boxes.size() << "\n";
        std::cout << "[YoloV8OIV7Backend][debug] Detections after NMS: " << indices.size() << "\n";
    }
    ...
    for (int idx : indices) {
        ...
        if (frameCount_ < 5 && loggedDetections < 5) {
            std::cout << "[YoloV8OIV7Backend][debug] Detection " << loggedDetections << ":"
                      << " ClassId=" << d.ClassId
    ...
    if (frameCount_ < 5) {
        std::cout << "[YoloV8OIV7Backend] Motorcycles detected: " << motorcycleCount << "\n"
                  ...
                  << " Person=" << personCount
                  << " Estimated=" << estimatedMotorcycles
